...
public class ScraperScheduler {
    ...
    public void yahooFinanceScheduling() {
       log.info("scraping scheduler is started");
        ...
        // 스크래핑한 대방금 정보 줌 데이터베이스에 없는 값은 저장
        scrapedResult.getDividends().strean()
                // 디비든 모델을 디비든 엔티티로 매핑
                .map(e -> new DividendEntity(company.getId(), e))
                // 엘리먼트를 하나씩 디비든 레파지토리에 삽입
                .forEach(e -> {
                    boolean exists = this.dividendRepository.exitsByCompanyIdAndDate(e.getCompanyId(), e.getDate());
                    if (!exists) {
